...
class Users extends CI_Controller {
		...
		public function used_books()
	{
		...
		$config = [
			...
			'base_url' => base_url('users/used_books'),
			'per_page' => 18,
			'total_rows'=>  $this->user_model->num_rows_books(),
			'full_tag_open' => "<ul class='custom-pagination'>",
			'full_tag_close' => "</ul>", 
		...
		$this->load->model('user_model');
		$view['books'] = $this->user_model->get_usedbooks($config['per_page'], $this->uri->segment(3));

		$view['user_view'] = "users/used_books";
		$this->load->view('layouts/user_layout', $view);
	}
	...
	public function best_books()
	{
		...
		$config = [
			...
			'base_url' => base_url('users/best_books'),
			'per_page' => 18,
			'total_rows'=>  $this->user_model->num_rows_bestbooks(),
			'full_tag_open' => "<ul class='custom-pagination'>",
			'full_tag_close' => "</ul>", 
			'first_tag_open' => '<li>',
			'first_tag_close' => '</li>',
		...
		$this->load->model('user_model');
		$view['books'] = $this->user_model->get_bestbooks($config['per_page'], $this->uri->segment(3));

		$view['user_view'] = "users/best_books";
		$this->load->view('layouts/user_layout', $view);
	}
