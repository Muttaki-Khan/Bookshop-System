...
/*================ Books &&  All Books list page ===============*/
		...
		public function bestbooks()
	{
		...
		$config = [
			...
			'base_url' => base_url('admin/bestbooks'),
			'per_page' => 10,
			'total_rows'=>  $this->admin_model->num_rows_best_books(),
			'full_tag_open' => "<ul class='custom-pagination'>",
			'full_tag_close' => "</ul>", 
			'first_tag_open' => '<li>',
			'first_tag_close' => '</li>',
		...
		$this->load->model('admin_model');
		$view['books'] = $this->admin_model->get_bestbooks($config['per_page'], $this->uri->segment(3));

		$view['admin_view'] = "admin/bestsellerbooks";
		$this->load->view('layouts/admin_layout', $view);
	}
...
/*================ Add Books Page =================*/
	...
	public function usedbook_edit($id)
	{
		...
		if(($this->form_validation->run() && $this->upload->do_upload()) == FALSE)
		{
			...
			if($this->admin_model->get_usedbook_detail($id))
			{
				$view['admin_view'] = "admin/usedbook_edit";
				$this->load->view('layouts/admin_layout', $view);
			}
			else
			{
		...
		else
		{
			...
			if($this->admin_model->edit_usedbook($id, $data))
			{
				$this->session->set_flashdata('success', 'Used Book info update successfully');
				redirect('admin/usedbooks');
			}
			else
			{
